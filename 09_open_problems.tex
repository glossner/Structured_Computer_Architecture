...
While the hardware's size accommodated on a single die of silicon increases exponentially, according to Moore's Law \cite{Moore, Moore1}, its use for increasing the complexity and the intensity of computation grows much slowly. We are able to build big machines but we fail in using all their computational power efficiently. For example:
\begin{itemize}
    \item  Tensor Processing Unit, is reported for $>90\%$ of the applications  with the use of $3\%$ to $13.4\%$ from its peak performance \cite{Jouppi} \cite{Hennessy1}. Only for one application (involving exclusively convolutional layers in a deep neural network) used in less than $5\%$ of cases, $93\%$ from its peak performance is used
    \item the real time image recognition, where Nvidia's Titan X GPU, uses maximum 63 GFLOPs/sec from its peak performance of 6 TFLOPs/sec \cite{Redmon}
    \item Intel's Xeon Phi  accelerator with 57 cores, having  peak performance at 2 TFLOPs/sec, involves only 35.2 GFLOPs/sec for a similar task \cite{Raina}.
\end{itemize}
...
\placedrawing[H]{figures/09_01_why_parallelism.lp   F67.lp}{Why Parallelism? Because only the transistors number goes up \cite{MooreC}.}{whyPar}
...
After the year 2000, the clock speed race started to slowdown \cite{Asanovic06, Asanovic09}. Instead of a clock with increasing frequency, the number of cores per chip started to grow with the hope that the programmer will find a way to take out more performance form the silicon maintaining the clock frequency around  of few GHz.  The situation is very clearly expressed by David Patterson:
\begin{quote}
$\ldots$ {\em the semiconductor industry threw the equivalent of a Hail Mary pass when it switched from making microprocessors run faster to putting more of them on a chip -- doing so without any clear notion of how such devices would in general be programmed. The hope is that someone will be able to figure out how to do that, but at the moment, the ball is still in the air}. \cite{Patterson}
\end{quote}
...
\begin{itemize}
  ...
  \item 1900-1928: David Hilbert \cite{Hilbert, Hilbert28} reformulated rigorously the problem as the {\bf {\em decision problem}}
  \item 1931: Kurt G\" odel \cite{Godel} proved that the decision problem, and consequently the liar paradox, has no a logic solution
  \item 1936: Alonzo Church \cite{Church}, Stephene Kleene \cite{kleene}, Emil Post \cite{Post}, Alan Turing \cite{Turing} published independently their mathematical version of G\" odel's approach thus providing four equivalent {\bf {\em mathematical models}} for computing as a mechanism based on logic decision
  \item 1937: Claude E. Shannon \cite{Shannon48} defended at MIT its master thesis which became the foundation of practical digital circuit design during and after World War II
  \item 1946: John von Neumann \cite{von Neumann}, based on the Turing's approach and on the design and implementation made by John Mauchly and J. Presper Eckert for ENIAC, provided the {\bf {\em abstract model}} for a mono-core computing machine (his approach  is paralleled  by the {\em Harvard} version)
...
and controls, the logical design, and the physical implementation"} \cite{Amdahl}, is introduced when specifying the IBM 360 computer.
\end{itemize}
...
\begin{itemize}
    ...
    \item 1965-75 -- {\bf {\em architectural issues}}: Edsger W. Dijkstra formulates, starting with  \cite{Dijkstra}, the first concerns about specific parallel programming issues (such as critical regions problem, semaphores, the dining philosophers problem, guarded commands)
    \item 1974-82 -- {\bf {\em abstract machine models}}: proposals of the first abstract models (bit vector  models in \cite{Pratt74} and PRAM models in \cite{Fortune}, \cite{Goldschlage}) start to come in after almost two decades of non-systematic experiments (started in the late 1950s) and the too early market production
    \item {\bf ?} -- {\bf {\em mathematical parallel computation model}}: no one yet really considered it as mandatory, regrettably confusing it with abstract machine models, although it is there and wait to be considered (see Kleene's mathematical model for computation  \cite{kleene}).
\end{itemize}
...
The systems with global loops can be related with the model of {\em partial recursive functions} proposed by Stephen Cole Kleene \cite{kleene}.
{\small
\begin{shaded*}
%\begin{lstlisting}[language=, caption= , morekeywords={define}]
\begin{quote}
...
Let be the positive integers $x,y,i \in {\bf N}$ and the sequence $X = \langle x_0, x_1, \ldots, x_{n-1} \rangle \in {\bf N}^n$. Any partial recursive function $f: {\bf N}^n \rightarrow {\bf N}$ can be computed, according to \cite{kleene}, using three {\bf {\em initial functions}}:
\begin{itemize}
...
Starting from the suggestion offered by the representation in Figure \ref{compCirc} one can conceive, based on the mathematical model of partially recursive functions, an abstract model for what a parallel computing system must be. A first approach is outlined in \cite{Stefan14}.
