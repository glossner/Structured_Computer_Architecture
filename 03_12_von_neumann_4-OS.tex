...
\subsection{Turing Machine \& Related Work}

In his paper, about "computable numbers" and Entscheidungsproblem, Alan Turing describes in words what he called a {\em computing machine}. We call it now Turing Machine (TM) and we describe it as a structure with three components:
\begin{itemize}
    \item an infinite {\bf band} of paper on/from which symbols belonging to a finite set of symbols can be write or read using
    \item an accessing read/write {\bf head} under the control of
    \item a {\bf finite automaton} (FA)
\end{itemize}
as is represented in Figure \ref{tm}. 

\placedrawing[H]{figures/12_02_turingMachine.lp}{Turing Machine. {\bf a.} Original description: Band \& Head \& FA. {\bf b.} Digital representation: Memory \& upDownConter \& FA.}{tm}
