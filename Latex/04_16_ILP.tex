\chapter{Instruction-Level Parallelism\index{ILP (Instruction-Level Parallelism): }}\label{lect5}
\localtoc

There are various form of instruction-level parallelism. We will consider  only the parallelism introduced by the pipeline mechanism and the multiple execution units.
Thus, chapter deals with the pipeline mechanism, used to accelerate the processor's operation, and the implications of its use. But as we will see any gain obtained from the application of an improvement technique comes with a price.
...
If we process a sufficiently long string of data, {\tt X(1), X(2), ..., X(n)}, then the additional latency introduces an insignificant effect: the processing of the string will be performed in $n+3$ cycles instead of $n+2$ but at a frequency $\alpha$ times higher.

In other words, the introduction of the pipeline register in Figure \ref{fig47}b triggers a parallel process by which the system is involved in processing two successive components of the sequence to be processed. We a
...
There are three types of dependencies\index{dependencies!}:
\begin{enumerate}
...
Data dependency  solved by using forwarding is exemplified starting from Example \ref{dataDepSimulation}:
\begin{verbatim}
            NOP         ;
            VAL(0,1)    ;
            VAL(1,2)    ;
...
\subsubsection{Static Branch Prediction\index{branch prediction!}}
...
Dynamic branch prediction\index{branch prediction!dynamic branch prediction} predicts branches based on dynamic information collected at run-time. It requires additional hardware. As examples, I have chosen two of the simplest prediction mechanisms.
...
\subsubsection{Tomasulo's Algorithm\index{Tomasulo's algorithm:  is an algorithm for dynamic scheduling of instructions in out-of-order execution which enables more efficient use of multiple execution units}}\index{Tomasulo, Robert  (1934-2008): an American computer scientist, and the inventor of the Tomasulo algorithm}

The execution unit of a scalar processor has, in addition to the ALU for integers that we discussed, several units that perform operations with floating-point numbers (see Chapter \ref{floarReprezentation} in this book). For out-of-order execution, it is not enough to have a register file. Next to each execution unit, for integer or real, complex storage units called Resevation Stations (RS) will be added. Each recording in a RS has the following fields \citep{Patterson19}:
\begin{itemize}
