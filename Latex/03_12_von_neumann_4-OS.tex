\chapter{Computers\index{computers:  fourth order digital systems (4-OS) with the fourth loop  closed over a 3-OS}: Four-Loop, Fourth-Order Systems (4-OS)\index{4-OS: fourth order system -- interpreting computer}}\label{lect6}
\localtoc
...
\subsection{Memory Hierarchy\index{memory hierarchy}}\label{memhier}
...
\subsection{Virtual Memory Mechanism\index{virtual memory mechanism}}\label{lb:vmm}
...
\subsubsection{Translation Lookaside Buffer\index {TLB (Translation Lookaside Buffer)} (TLB)}
...
\subsection{DMA\index{DMA (Direct Memory Access)}}
...
initG   - processor loads (comA=11) in AddrsCounter the starting address 
...
\begin{itemize}
    \item Storage devices are mainly represented by hard disks, flash memory, optical disks, tape.
    \item Functional devices, mainly represented by display systems, network interfaces, ...
\end{itemize}
We will briefly present the storage devices that extend the memory hierarchy outside the central processing unit.

\subsubsection{Hard Disk Drives (HDDs)\index{HDD (Hard Disk Drive)}}
...
\subsubsection{Solid State Drives (SSD)\index{SSD (Solid State Drive)}}


Flash memory is a type of EEPROM (electronically erasable programmable read only memory), which is normally read only but can be erased. Solid-state devices (SSDs) based on flash memory technology do not have moving parts.

The capacity per flash memory chip increased by about 50\%–60\% per year.

Typical SSDs will have the time taken for a disk drive to locate the area on the disk where the data to be read is stored between 0.08 and 0.16 ms.

Time to {\bf read} 2 KB from flash memory takes about 75 $\mu$S,  while DDR SDRAM takes less than 500 ns. Then,  flash memory is  about 150 times slower. But compared to HDD is $\sim 400$  times faster. We conclude that flash memory cannot replace DRAM for main memory, but it is a good candidate to replace HDD.
...
\begin{problem}
Design a page translation system based on an associative memory (see \ref{lb:cam} in this book). The principle is describe in section \ref{lb:vmm}. Consider pages of 1KW in a physical space of 1024 pages and a virtual space of $1024^2$ pages.

$\diamond$
\end{problem}

\begin{problem}
Let us consider the simple DMA illustrated in Figure \ref{simpleDMA} with the behavior described in Example \ref{minDMA}. Design it in System Verilog and simulate in Vivado environment.
