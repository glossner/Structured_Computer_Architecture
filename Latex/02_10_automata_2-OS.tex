...
An initial block is dedicated to clock generation. The clock is initialized to 0, and a \texttt{forever} loop toggles the clock every time unit by inverting its value. This produces a periodic square wave that drives the synchronous components of the design. Another initial block manages the reset signal and input stimulus. Here, the reset is initially asserted, and the input is set to the value defined by the macro \texttt{\`a}. After a delay, the reset is de-asserted, and subsequent delays cause the input to change first to the value corresponding to \texttt{\`b} and later back to the value defined by \texttt{\`a}. The simulation is programmed to finish after a certain period, ensuring that all intended transitions are exercised.
