...
\section{Controlled Cellular Automaton}\label{contrCellAut}
...
\section{{\bf First Global Loop} in CA}
...
\section{{\bf Second Global Loop} in CA}
...
\begin{exemplu}
In Table \ref{prim} is listed an example for a library of primitives used to develop in a high level language on HOST a linear algebra library. The library of primitives contains functions defined on data structures, vectors and matrices, limited to the size of the array of cells  Map, i.e., $p$-component vectors and $p \times p$ matrices. Programs developed on HOST adapt linear algebra operations defined on data of any size to linear algebra operations defined on accelerator dimensions. In this way, a speedup is obtained that depends on $p$ even if the order of magnitude of the computation time remains the same, i.e., the execution time of matrix multiplication, for example, remains the same, in $O(n^3)$ for matrices of $n \times n$.

{\em
\begin{table}[H]
 
	\centering                       
	\caption{Library of linear algebra primitives for $p \times p$ matrices and $p$-component vectors on a $p$-cell MapScanReduce accelerator. }           
	\label{prim}       % Label

	\begin{tabular}{|p{0.2\textwidth}|p{0.6\textwidth}|p{0.1\textwidth}| } \hline  % Your table
		{\bf Function} & {\bf Description} & {\bf Time}  \\ \hline \hline
        vLoad(Vi)              & load vector Vi from the HOST's memory &  $O(p)$ \\ \hline
        vStore(Vi)             & store vector Vi to the HOST's memory &  $O(p)$     \\ \hline
        mLoad(Vi,s)            & load from the HOST's memory the s-line matrix starting with Vi &  $O(p^2)$    \\ \hline
        mStore(Vi,s)           & store to the HOST's memory the s-line matrix starting with Vi & $O(p^2)$   \\ \hline
        vvMult(Vi,Vj,Vk) & inner product in Vi between Vj and Vk & $O(log \; p)$ \\ \hline
		mvMult(Vi,Vj,Vk) & multiply in Vi the $p \times p$ matrix starting with Vj, with vector Vk  &  $O(p)$   \\ \hline
        mmMult(Vi,Vj,Vk) &  multiply in the $p \times p$ matrix starting with Vi, the  $p \times p$ matrices starting with Vi and Vk &  $O(p^2)$ \\ \hline
         mmMac(Vi,Vj,Vk) &  multiply and accumulate in the $p \times p$ matrix starting with Vi, the  $p \times p$ matrices starting with Vi and Vk &  $O(p^2)$ \\ \hline
        mTrans(Vi,Vj) & transpose in the $p \times p$ matrix starting with Vi the  $p \times p$ matrix starting with Vi &  $O(p)$  \\ \hline
	\end{tabular}
