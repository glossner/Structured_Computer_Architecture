...
\begin{table}[H]
	...
	\begin{tabular}{|p{0.2\textwidth}|p{0.7\textwidth}|} \hline  % Your table
		Function & Description  \\ \hline \hline
        vLoad(Vi)              & load vector Vi from the HOST's memory   \\ \hline
        vStore(Vi)             & store vector Vi to the HOST's memory     \\ \hline
        mLoad(Vi,s)            & load from the HOST's memory the s-line matrix starting with Vi    \\ \hline
        mStore(Vi,s)           & store to the HOST's memory the s-line matrix starting with Vi     \\ \hline
        vvMult(Vi,Vj,s,Vk) & inner product in Vi between Vj and Vk  \\ \hline
		mvMult(Vi,Vj,s,Vk) & multiply in Vi the s-line matrix starting with Vj, with vector Vk    \\ \hline
        mmMult() &   \\ \hline
        mTrans() &  \\ \hline
	\end{tabular}
