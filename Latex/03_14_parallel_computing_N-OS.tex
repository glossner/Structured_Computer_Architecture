...
for(k=0; k<m; k=k+1) begin
  (1) A <= [v(0+k,0) v[1+k,1]...v[m-1+k,m-1] // A <= PD(k.V)
  (2) A <= [a(0-k) a(1-k) ... a(m-1-k)]      // A <= (A rotate k positions right)
  (3) for(j=0; j<m; j=j+1) w(j+k,j) <= a(j)  // PD(m-k,W) <= A
end
\end{lstlisting}
\end{shaded*}
}
...
\begin{verbatim}
     Initial                                                          Final
                           k = 0    k = 1    k = 2    k = 3
 V:  0 1 2 3                                                     V:  0 1 2 3
     4 5 6 7     (step 1) 0 5 A F  4 9 E 3  8 D 2 7  C 1 6 B         4 5 6 7
     8 9 A B                                                         8 9 A B
     C D E F     (step 2) 0 5 A F  3 4 9 E  2 7 8 D  1 6 B C         C D E F
