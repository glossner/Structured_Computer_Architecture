...
SystemVerilog is a hardware description and hardware verification language, which extends the capabilities of Verilog by incorporating advanced modeling features and constructs \cite{ieee1800-2017}. It combines design specification and verification features, making it a unified language for hardware design and verification. This chapter serves as a reference guide for hardware designers familiar with Verilog or VHDL, providing syntax and commands of SystemVerilog for quick consultation, and highlighting the differences between Verilog and SystemVerilog.
...
\begin{itemize}
    \item \texttt{logic}: Replaces \texttt{reg} and can be driven by continuous assignments.
    \item \texttt{bit}: Unsigned, 2-state data type (0 or 1).
    \item \texttt{byte}: 8-bit signed integer.
    \item \texttt{shortint}: 16-bit signed integer.
    \item \texttt{int}: 32-bit signed integer.
    \item \texttt{longint}: 64-bit signed integer.
    \item \texttt{integer}: Equivalent to \texttt{int}.
    \item \texttt{time}: 64-bit unsigned integer.
\end{itemize}

\subsubsection{User-Defined Types}

\begin{itemize}
    \item \texttt{typedef}: Used to create new type names.
        \newline Example: \texttt{typedef logic [7:0] byte\_t;}
    \item \texttt{enum}: Enumerated types.
        \newline Example: \texttt{enum logic [1:0] \{IDLE, BUSY, DONE\} state;}
    \item \texttt{struct}: Composite data types grouping variables.
        \newline Example: \texttt{struct \{logic [7:0] addr; logic [15:0] data;\} packet;}
    \item \texttt{union}: Overlapping data storage.
        \newline Example: \texttt{union \{int i; byte b[4];\} u;}
\end{itemize}
...
\begin{itemize}
    \item \textbf{Packed Arrays}: Continuous set of bits.
        \newline Declaration: \texttt{logic [7:0] data\_byte;}
    \item \textbf{Unpacked Arrays}: Arrays of elements.
        \newline Declaration: \texttt{logic [7:0] memory [0:255];}
\end{itemize}

\subsubsection{Dynamic and Associative Arrays}

\begin{itemize}
    \item \texttt{dynamic array}: Size can change at runtime.
        \newline Declaration: \texttt{int dynamic\_array[];}
    \item \texttt{associative array}: Indexed by arbitrary data types.
        \newline Declaration: \texttt{int associative\_array[string];}
    \item \texttt{queue}: Variable-size ordered collection.
        \newline Declaration: \texttt{int queue[\$];}
\end{itemize}
...
\begin{itemize}
    \item \textbf{Streaming Operators}: \texttt{<<}, \texttt{>>}
        \newline Used for packing and unpacking bits.
    \item \textbf{Type Casting}: \texttt{\$cast}, static casting (\texttt{type'(expression)})
    \item \textbf{Conditional Operator}: Ternary operator (\texttt{? :}) enhanced for 4-state expressions.
\end{itemize}
...
\begin{itemize}
    \item \texttt{always\_comb}: Replaces \texttt{always @(*)} for combinational logic.
    \item \texttt{always\_ff}: For sequential logic triggered by clock edges.
    \item \texttt{always\_latch}: For latch inference.
\end{itemize}
...
\begin{itemize}
    \item \texttt{foreach}: Iterates over arrays.
        \newline Example: \texttt{foreach (array[i]) array[i] = 0;}
    \item \texttt{do...while}: Post-test loop.
    \item \texttt{break}, \texttt{continue}: Loop control.
    \item \texttt{return}: Exits a function or task.
\end{itemize}

\subsubsection{Conditional Compilation}

\begin{itemize}
    \item \texttt{`\$ifdef}, \texttt{`\$ifndef}, \texttt{`\$elsif}, \texttt{`\$else}, \texttt{`\$endif}
\end{itemize}
...
Interfaces encapsulate communication between modules \cite{guyer2005introduction}.
...
SystemVerilog provides significant enhancements over Verilog, offering advanced features for both design and verification. Understanding the differences and new constructs is essential for hardware designers to effectively utilize the language's capabilities.
