...
//************ MATRIX-VECTOR MULTIPLY ***********************
    LB(12); cPARAM;         NOP;
            cPARAM;         CLOAD;
            cSTORE(0);      VADD(1);    // mem[0] = in vector addr
            cPARAM;         ADDRLD;     // matrix end addr + 1
            cSTORE(1);      REDADD;     // mwm[1] = out vector addr
            cVLOAD(16);     RILOAD(-1);
    LB(24); cLREDINS;       MULT(16);
...
//********** MULTIPLY SQUARE MATRICES ***********************
    LB(13); cPARAM;         REDADD;
            ...
            cVADD(1);       CALOAD;
            cSTORE(2);      STORE(3*`p);
            cVLOAD(`p + 1.5*$clog2(`p) - ((($clog2(`p))/2)*2) - 2);     RILOAD(0);
    LB(25); cLREDINS;       MULT(3*`p);
            cBRNZDEC(25);   RILOAD(-1);
            cLOAD(3);       NOP;
            cVADD(1);       NOP;
            cSTORE(3);      GETSR;
            cLOAD(2);       CSTORE;  // CSADD;
            cVADD(1);       CALOAD;
            cSTORE(2);      NOP;
            cLOAD(0);       STORE(3*`p);
            cLOAD(1);       CLOAD;
            ...
            cBRZDEC(32) ;   ADDRLD;
            cSTORE(1);      NOP;
            cVLOAD(`p + 1.5*$clog2(`p) - ((($clog2(`p))/2)*2) - 2);     RLOAD(0);
            cJMP(25);       NOP;
