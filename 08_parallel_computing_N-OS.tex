...
The are the following subsets of operations performed, under the command of SQ, in the Map section. 
...
Multiplying matrices is a promising example for the capacity to perform in parallel with MapScanReduce accelerator intense computation. It would be interesting to investigate the acceleration capacity of the MapScanReduce framework for a wider range of applications. A first idea could be formed by looking at the computational patterns that have been highlighted in applications designed and run in the last decades. A good start can be the study carried out in \cite{McCool12} from which we use Figure \ref{parallelPatt}.
