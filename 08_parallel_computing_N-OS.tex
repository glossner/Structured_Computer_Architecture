...
\placedrawing[h]{figures/09_07_controlledCA.lp}{Controlled Cellular Automaton (CCA).}{controledCA}
...
\begin{itemize}
 ...
 \placedrawing[h]{figures/09_11_distribution_net.lp}{Distribution network for a circuit with $p = 16$.}{distribute}
    ...
    \item $c_0, c_1, \ldots, c_{p-1}$ form a cellular automaton with a finite number of cells, each equipped with a RALU type execution unit
\end{itemize}
...
{\bf M_{mp}} = \begin{bmatrix}
    ...
    \end{bmatrix}
\]
whose columns represent the local memory (register file) in each cell. SQ issues operations applied to the line of the matrix {\bf M} seen as a vector, 
$$V_i = [s_{i0} \; s_{i1} \; \ldots \; s_{i(p-1)}]$$ for $i = 0, \ldots, m-1$ distributed along the cells, so that an operation of the form:
...
\placedrawing[h]{figures/09_12_reduce_net.lp}{Reduce network for $p=16$, with the size in $O(p)$ and the depth in $O(log \; p)$.}{reduceNet}
    ...
    \item Reduce: a pipelined $log$-depth network (see Figure \ref{reduceNet}  which reduce the vector received from Map to a scalar sent to SQ with a $log$ latency; it performs usually few functions, such as:
        $$sumReduce(V_i) = \sum_{j=0}^{p-1} s_{ij}$$
        $$minReduce(V_i) = MIN(s_{i0}, s_{i1}, \ldots, s_{i \; p-1})$$
       $$maxReduce(V_i) = MAX(s_{i0}, s_{i1}, \ldots, s_{i \; p-1})$$
       $$orReduce(V_i) =s_{i0} \; | \; s_{i1} \; | \; \ldots \; | \;s_{i \; p-1}$$
\end{itemize}
...
\begin{exemplu}
To calculate the scalar product of two vectors in such a system, the following is performed, under the SQ command:
$$V_i \cdot V_j = sumReduce(V_i \times V_j)$$
...
let's define the two types of computations. In this respect, we use the {\em algorithmic complexity} \cite{Chaitin} which helps us make the necessary distinction between size and complexity.
...
will be able to figure out how to do that, but at the moment, the ball is still in the air."} \cite{Patterson}
\end{quote}
}
