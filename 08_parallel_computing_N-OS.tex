...
\subsection{Controlled Cellular Automaton}
...
\paragraph{Spatial selection}  operations are used to modify the Boolean vector 
$B=\langle
\begin{matrix}
...
\paragraph{Arithmetic \& Logic vector operations}  are performed on active components of vectors belonging to the active cells where $b_i=1$. Generic forms:
\begin{description}
...
\paragraph{Global shift/rotate operations} performed on all components of vectors distributed along the cells in Map seection. For example:
\begin{description}
...
\begin{itemize}
    ...
    \item Reduce: a pipelined $log$-depth network (see Figure \ref{reduceNet}  which reduce the vector received from Map to a scalar sent to SQ with a $log$ latency; it performs usually few functions, such as:
    \begin{description}
        \item[{\tt ADDRED(s)}:] reduction add \\
        $sqAcc \Leftarrow \sum_{j=0}^{p-1} s_{ij}$
        \item[{\tt MINRED(s)}:] reduction minimum \\
        $sqAcc \Leftarrow MIN(s_{i0}, s_{i1}, \ldots, s_{i \; p-1})$
        \item[{\tt MAXRED(s)}:] reduction maximum\\
        $sqAcc \Leftarrow MAX(s_{i0}, s_{i1}, \ldots, s_{i \; p-1})$
        \item[{\tt ORRED(s)}:] reduction or\\
        $sqAcc  \Leftarrow s_{i0} \; | \; s_{i1} \; | \; \ldots \; | \;s_{i \; p-1}$
    \end{description}
    where $sqAcc$ is the accumulator register in SQ.
\end{itemize}
...
\begin{exemplu}
To calculate the scalar product of two vectors, $V_i$ and $V_J$, in such a system, the following is performed, under the SQ command:
\begin{verbatim} 
                     sqAcc <= ADDRED(MULT(i,j))  
\end{verbatim}
