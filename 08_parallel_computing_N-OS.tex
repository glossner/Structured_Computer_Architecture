...
The state of SQ is represented by an array of scalar values: {\tt MSQ = [v0 v1 ... ]}. 
...
The geometric decomposition pattern works on configurable (overlapping) sub-collections  of data. In Figure \ref{parallelPatt}, four $4 \times 5$ scalars sub-collections are defined. If data is loaded in the accelerator's memory as a matrix of scalars, a sub-collection, $S(i,j,k,q)$, is a matrix loaded in ${\bf M_{mp}}$ between two horizontal vectors , $V_i$ and $V_j$, and two vertical vectors, $C_k$ and $C_q$. The values for $i$ and $j$ are controlled by the program executed on the sequencer SQ, while the values of $k$ and $q$ are used by the {\em spatial selection} mechanism in Map. If  $A \Leftarrow \langle i  \; i  \; \ldots  \; i \rangle$, then  the program can be written using  $BOP(d,l,r)$ or $UOP(d,l)$ for $d,l,r \in [1,j]$. The $IX$ vector is used to delimit the index  of the vertical vectors applying spatial selection functions:
\begin{verbatim}
    WHERE(IX>=k)
    WHERE(IX<=q)
    LOAD(A,i)   // A <= <i,i,...,i>
    MAPOP(0,1,...,j-1,j)
\end{verbatim}
Thus, any sub-collection is delimited in the matrix ${\bf M_{mp}}$, if $i,j,k,q \leq m,p$, in constant time.

If the sub-collections do not overlap, the computation can be done on all of them in parallel as, for example, for computing convolution  with a $f \times f$ components filter can be performed on $p/f$ $f \times f$ sub-collections in parallel, obtaining an acceleration in $O(p)$.
